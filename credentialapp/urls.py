...
urlpatterns = [
    ...
    # ==================== CERTIFICATES — static paths FIRST ====================
    # Static/action paths must come before any dynamic <int:certificate_id> patterns.
    ...
    # ── Public verification — static "search/" before dynamic "<str:certificate_no>/" ──
    # NOTE: parameter is "certificate_no", not "credential_id".
    #       The view function signature must match: verify_certificate(request, certificate_no)
    path('certificate/verify/search/', search_certificate, name='search_certificate'),
    path('certificate/verify/<str:certificate_no>/', verify_certificate, name='verify_certificate'),

    # ── Dynamic certificate routes (integer ID) ──
